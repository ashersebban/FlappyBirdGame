...
void drawBird(PVector pos){
  ...
  //BIRDS WING
  fill(255);
  ellipse(pos.x-birdWidth*0.35,pos.y+(sin(f)*5)-2,0.5*birdWidth,0.4*birdHeight);
  //BIRDS EYE
...
boolean collision(){
  ...
  //VARS PLACED HERE FOR YOUR CONVINIENCE
  ...
  //if the bird is in the area where the pipe is
  //but not in the pipe gap
  //COLLISSION!
...
void scoreBoard(){
  ...
  //HIGHSCORE
  rectMode(CENTER);
  if(highscoreReached)fill(255, 243, 20);
